...
const proxyToLocalhost = (req, res, next) => {
    try {
        ...
        const config = { target, prependPath: true, ignorePath: true, xfwd: true, secure: false, cookieDomainRewrite: { "localhost:3000": `localhost:${port}` }};
...
app.all('*',(req, res) => {
    try {
        ...
        proxy.web(req, res, { target: `https://gigabowser.now.sh`, changeOrigin: true, cookieDomainRewrite: { "tamudatathon.com": `localhost:${port}` }})
    } catch(e) {
        console.error(e);
        res.status(500).send();
    }
