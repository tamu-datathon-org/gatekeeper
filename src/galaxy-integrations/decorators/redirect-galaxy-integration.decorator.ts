...
import { DefaultGalaxyIntegrations, GatekeeperGalaxyIntegration } from "../constants/default-integrations";
...
const getLongestMatchingPrefix = (key, prefixes) => {
  ...
  prefixes.forEach((prefix) => {
    if (key.startsWith(prefix)) {
      if (prefix.length > longestPathPrefix.length)
...
*/
export const RedirectGalaxyIntegration = 
createParamDecorator((data: unknown, ctx: ExecutionContext) => {
    const request = ctx.switchToHttp().getRequest();
    const redirectPath = request.query?.r || "/auth/me";

    // Use default gatekeeper pathPrefix if no matching prefix.
    const longestPathPrefix = getLongestMatchingPrefix(redirectPath, Object.keys(DefaultGalaxyIntegrations)) 
