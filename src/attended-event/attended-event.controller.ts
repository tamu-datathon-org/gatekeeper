import { Controller, Get, UseGuards, Query, Res, Post, Body, BadRequestException, UnauthorizedException } from '@nestjs/common';
...
export class AttendedEventController {
  ...
  async getAttendees(@Query("eventId") eventId, @Res() res) {
    const attendedObjects = await this.attendedEventService.findAll({eventId});
  ...
  async getAttendedEvents(@Query("userAuthId") userAuthId, @Res() res) {
    const attendedObjects = await this.attendedEventService.findAll({userAuthId});
    return res.json({data: attendedObjects});
  }
  ...
  @Post("/")
  async addAttendedEvent(@GetUser() user: User, @Body("eventId") eventId: string, @Body("userAuthId") userAuthId: string, @Res() res) {
    if(!userAuthId || !eventId) {
      throw new BadRequestException("Body must container eventId and userAuthId");
    }
    // Only admins can post attended events for other users.
    if (!user.isAdmin && (user.authId !== userAuthId)) {
      throw new UnauthorizedException("User can't post attended events for others.");
    }
    const attendedEvent = (await this.attendedEventService.record({eventId, userAuthId})).toObject();
