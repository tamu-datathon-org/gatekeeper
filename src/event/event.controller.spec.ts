...
describe('Event Controller', () => {
  ...
  it("should be able to create a new event", async () => {
    ...
    const event = await controller.createEvent("Test Name", "Description", "parent-id");
  ...
  it("should fail to create event when eventName is missing", async () => {
    const promise = controller.createEvent(undefined, "Description", "parent-id");
  ...
  it("should allow creating events without eventDescription and parentId", async () => {
    ...
    const event = await controller.createEvent("Test Name", undefined, undefined);
    expect(event.name).toEqual("Test Name");
  });

  it("should get an event successfully", async () => {
    jest.spyOn(eventService, "findById").mockImplementation(
      async (id): Promise<Event> => ({
          name: "Test Name",
          parentId: "parent-id",
          description: "desc"
        } as Event)
  ...
  it("should update an event successfully", async () => {
    jest.spyOn(eventService, "update").mockImplementation(
      async () => { return; }
    );
    const result = await controller.modifyEvent("event-id", "name", "desc", "parent-id");
    expect(result.success).toEqual(true);
  });

  it("should fail to modify event when eventId is missing", async () => {
    const promise = controller.modifyEvent(undefined, "name", "desc", "parent-id");
    await expect(promise).rejects.toThrow(BadRequestException);
  });
  ...
  it("should propagate errors for EventService when modifying events", async () => {
    ...
    const promise = controller.modifyEvent("random-id", "name", "desc", "parent-id");
    await expect(promise).rejects.toThrow(Error);
  });

  it("should partially update events when a subset of the fields are provided", async () => {
    jest.spyOn(eventService, "update").mockImplementation(
      async (id: string, fields: UpdateQuery<Event>) => {
    ...
    const result = await controller.modifyEvent("random-id", "name", undefined, "parent-id");
